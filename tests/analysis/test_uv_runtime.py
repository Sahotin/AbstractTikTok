...
def test_uv_environment_preserves_explicit_overrides(monkeypatch) -> None:
    monkeypatch.setenv("UV_CACHE_DIR", "D:/custom-cache")
    monkeypatch.setenv("UV_PYTHON_INSTALL_DIR", "D:/custom-python")

    environment = build_uv_environment()

    assert environment["UV_CACHE_DIR"] == "D:/custom-cache"
    assert environment["UV_PYTHON_INSTALL_DIR"] == "D:/custom-python"
