...
from sqlalchemy import case, event, func, or_, select, true, update
...
from database.semantic_models import SemanticRunItemModel
...
class AnalysisRepository:
    ...
    async def list_collection_runs(
        ...
        *,
        ...
        statement = select(CollectionRunModel)
        if platform:
            statement = statement.where(CollectionRunModel.platform == platform)
        statement = statement.order_by(
            CollectionRunModel.finished_at.desc().nullslast(),
            CollectionRunModel.started_at.desc(),
        ).limit(limit)
        async with self._session_factory() as session:
            rows = (await session.execute(statement)).scalars().all()
        return [_orm_to_domain(row, CollectionRun) for row in rows]

    async def count_analyzed_comments_for_run(self, run_id: str) -> int:
        statement = (
            select(func.count(func.distinct(CommentAnalysisModel.comment_id)))
    ...
    async def latest_analysis_job_for_run(self, run_id: str) -> AnalysisJob | None:
        statement = (
            select(AnalysisJobModel)
            .where(AnalysisJobModel.run_id == run_id)
    ...
    def _latest_analysis_cte(
        ...
        *,
        ...
        run_id: Optional[str] = None,
        content_id: Optional[str] = None,
        semantic_run_id: Optional[str] = None,
    ) -> Any:
        ...
        if semantic_run_id is not None:
            # The semantic-run item is the durable snapshot boundary.  It
            # points to an exact analysis row, so an old/new LLM version or a
            # low-information comment cannot leak into this aggregate.
            statement = statement.join(
                SemanticRunItemModel,
                SemanticRunItemModel.analysis_id == CommentAnalysisModel.id,
            ).where(SemanticRunItemModel.semantic_run_id == semantic_run_id)
        ranked = statement.where(*filters).cte("ranked_comment_analyses")
    ...
    async def get_public_opinion_counts(
        ...
        *,
        ...
        filters = self._comment_scope_filters(platform=platform, run_id=run_id, content_id=content_id)
        latest = self._latest_analysis_cte(platform=platform, run_id=run_id, content_id=content_id, semantic_run_id=semantic_run_id)
        async with self._session_factory() as session:
    ...
    async def get_comment_volume_by_date(
        ...
        *,
        ...
        statement = (
            ...
            .select_from(NormalizedCommentModel.__table__.join(
    ...
    async def get_trend_buckets(
        ...
        *,
        ...
        statement = (
            ...
            .select_from(
                NormalizedCommentModel.__table__.join(
                    latest, latest.c.comment_id == NormalizedCommentModel.id
                ) if semantic_run_id is not None else
    ...
    async def list_high_risk_comments(
        ...
        *,
        ...
        if not 1 <= limit <= 100:
            raise ValueError("limit must be between 1 and 100")
        latest = self._latest_analysis_cte(
            platform=platform, run_id=run_id, content_id=content_id, semantic_run_id=semantic_run_id
        )
        statement = (
            select(
                NormalizedCommentModel,
    ...
    async def list_risk_evidence_comments(
        self,
        *,
        platform: Optional[str] = None,
        run_id: Optional[str] = None,
        ...
        """Return bounded negative/risky evidence from each comment's latest analysis."""
        ...
        severity = case(
            (latest.c.risk_level == "high", 3),
            (latest.c.risk_level == "medium", 2),
            (latest.c.sentiment == "negative", 1),
            else_=0,
        )
        statement = (
            select(
            ...
            .join(latest, latest.c.comment_id == NormalizedCommentModel.id)
            .where(or_(latest.c.risk_level.in_(["high", "medium"]), latest.c.sentiment == "negative"))
            .order_by(
                severity.desc(),
                NormalizedCommentModel.like_count.is_(None),
                NormalizedCommentModel.like_count.desc(),
    ...
    async def fail_pending_job_items(
        self,
        job_id: str,
        *,
        ...
        """Finish untouched items after a fatal provider rejection."""
        ...
        async with self._write_lock:
            async with self._session_factory.begin() as session:
                ...
                await session.execute(
                    ...
                    .values(
                        status=AnalysisJobItemStatus.FAILED.value,
                ...
                await session.execute(
                    ...
                    .values(
                        pending_count=AnalysisJobModel.pending_count - pending_count,
                        failed_count=AnalysisJobModel.failed_count + pending_count,
                    )
                )
