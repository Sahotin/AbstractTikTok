...
class TopicService:
    ...
    async def cluster_run(
        ...
        try:
            ...
            # Evaluate the whole fixed snapshot, not only rows that received
            # an embedding.  Low-information comments intentionally skip the
            # embedding provider; they must still be recorded as excluded so
            # the topic result explains why they are absent from clustering.
            for row in snapshot:
            ...
            await self.repository.replace_topic_results(
                ...
                run_updates={
                    ...
                    "algorithm_config": algorithm_config,
                    "config_hash": config_hash,
                    "clustering_input_comments": result.input_comments,
                    "clustered_comments": result.clustered_comments,
                    "noise_comments": len(result.noise_comment_ids),
                    "clustering_failed_comments": max(
                        result.input_comments - result.clustered_comments - len(result.noise_comment_ids), 0
                    ),
