...
from analysis.llm import LLMProviderError
...
class AnalysisJobRunner:
    ...
    async def run(self, job_id: UUID | str) -> AnalysisJob:
        ...
        try:
            ...
            fatal_provider_error = asyncio.Event()
            fatal_error_message: list[str] = []
            ...
            try:
                while True:
                    ...
                    if fatal_provider_error.is_set():
                        message = fatal_error_message[0] if fatal_error_message else "Provider rejected the request"
                        await self.repository.fail_pending_job_items(
                            job_id_string,
                            finished_at=utc_now(),
                            error_type="FatalProviderError",
        ...
        while True:
            ...
            try:
                ...
                if fatal_provider_error.is_set():
                    if await self.repository.claim_job_item(str(item.id), utc_now()):
                        await self.repository.finish_job_item(
                            ...
                            finished_at=utc_now(),
                            error_type="FatalProviderError",
                            error_message=fatal_error_message[0],
                        )
                    continue
                ...
                try:
                    ...
                    if type(exc) is LLMProviderError:
                        if not fatal_error_message:
                            fatal_error_message.append(str(exc)[:500])
                        fatal_provider_error.set()
            finally:
                queue.task_done()
