...
from analysis.preprocessing import normalize_text, prepare_analysis_text
...
class SemanticSearchService:
    ...
    async def search(self, query: SemanticSearchQuery) -> SemanticSearchResult:
        ...
        query_text = prepare_analysis_text(query.query)
        if not query_text:
        ...
        for row in snapshot:
            ...
            if query.risk_level is not None and (
                ...
                excluded += 1
                continue
            if query.sentiment is not None and (
                row.analysis is None or row.analysis.sentiment != query.sentiment
            ):
                excluded += 1
                continue
            if query.content_id is not None and row.comment.content_id != query.content_id:
                excluded += 1
                continue
            if query.start_at is not None and (
                row.comment.published_at is None or row.comment.published_at < query.start_at
            ):
                excluded += 1
                continue
            if query.end_at is not None and (
                row.comment.published_at is None or row.comment.published_at >= query.end_at
            ):
                excluded += 1
                continue
            rows.append(row)
        ...
        for candidate in candidates:
            row = candidate.row
            normalized = prepare_analysis_text(row.comment.text)
