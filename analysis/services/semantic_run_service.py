...
from analysis.preprocessing import compute_text_hash, prepare_analysis_text, utc_now
...
class SemanticRunService:
    ...
    async def create_run(
        ...
        *,
        ...
        eligible_comments = sum(bool(prepare_analysis_text(comment.text)) for comment in comments)
        run = SemanticAnalysisRun(
            ...
            excluded_comments=len(comments) - eligible_comments,
            analyzed_comments=(
    ...
    async def execute_run(self, run_id: UUID | str) -> tuple[SemanticAnalysisRun, EmbeddingReport]:
        ...
        try:
            ...
            # Empty/low-information inputs are intentionally skipped before
            # the provider. They are exclusions, not provider failures.
            embedding_failures = max(report.failed_comments - run.excluded_comments, 0)
            ...
            stored = await self.semantic_repository.update_semantic_run(
                ...
                embedding_failed_comments=embedding_failures,
                embedding_dimension=report.dimension,
                finished_at=utc_now(),
                error_message=(f"{embedding_failures} provider embedding(s) failed" if embedding_failures else None),
            )
            if stored is None:
    ...
    async def _persist_fixed_snapshot(
        ...
        hashes = {
            str(comment.id): compute_text_hash(prepare_analysis_text(comment.text))
