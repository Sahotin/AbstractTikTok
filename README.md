...
本分支在 MediaCrawler 的多平台采集能力之上增加了独立 AI 应用层。爬虫核心保持原结构，采集数据通过统一模型进入可测试、可复现的评论分析系统：

```text
MediaCrawler 数据 → 标准化 SQLite → DeepSeek 结构化评论分析
                 → 情感/立场/风险聚合 → 时间趋势与拐点
                 → Qwen Embedding → 语义主题与相似评论检索
                 → DeepSeek Tool Calling Agent → Dashboard
```

核心能力：

- 抖音内容、作者、一级/二级评论的统一数据模型与幂等导入；
- 有缓存、并发限制、重试、速率限制、失败隔离和成本估算的评论 LLM Pipeline；
- 情感、情绪、立场、风险、主题、关键词及交叉分布的 SQL 聚合；
- 固定快照、版本化真实 Embedding、低信息评论质量门和可复现实验报告；
- 主题聚类、代表性评论、语义检索、时间趋势与可解释风险分；
- 8 个只读工具组成的真实 Tool Calling Agent，具备作用域、步骤、重复调用和上下文限制；
- `/dashboard` AI 分析看板，同时保留 `/` 原采集控制台。
...
1. 检查 `uv`；
2. 首次运行时从 `.env.example` 创建本地 `.env`；
3. 按 `uv.lock` 同步依赖并执行数据库迁移；
4. 启动 FastAPI，并在服务就绪后打开采集控制台。
...
# 导入抖音 JSON / JSONL 数据
uv run python -m analysis.cli ingest --platform douyin --input data/douyin/json
...
- AI Dashboard：`http://127.0.0.1:8080/dashboard`
- OpenAPI：`http://127.0.0.1:8080/docs`

远程模型仅从环境变量读取密钥，密钥不会写入代码、数据库或报告。DeepSeek 当前使用 JSON Object 模式，Embedding 示例使用 SiliconFlow 的 Qwen 模型：
...
EMBEDDING_VERSION=qwen3_embedding_4b_v1
```

更多内容见 [AI 架构与运行手册](docs/AI_APPLICATION_ARCHITECTURE.md)、[Phase 3 验收报告](benchmarks/PHASE3_COMPLETION_REPORT.md)、[Phase 4A 质量评测报告](benchmarks/PHASE4A_COMPLETION_REPORT.md) 和 [简历与面试指南](docs/RESUME_INTERVIEW_GUIDE.md)。
