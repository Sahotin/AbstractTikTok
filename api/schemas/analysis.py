...
from analysis.agent import AgentRequest, AgentResponse, AgentToolTrace
...
class AgentChatFailureResponse(BaseModel):
    """Safe failure payload without prompts, secret configuration, or reasoning."""

    status: Literal["failed"] = "failed"
    error_type: str
    message: str
    partial_tool_calls: list[AgentToolTrace] = Field(default_factory=list)
...
class SemanticRunResponse(AnalysisResponseModel):
    ...
    clustering_input_comments: int
    clustering_failed_comments: int
...
class TopicClusterResponse(BaseModel):
    ...
    topic_naming_version: Optional[str] = None
    naming_provider: Optional[str] = None
    naming_model: Optional[str] = None
    naming_prompt_version: Optional[str] = None
...
class AnalysisResultScopeResponse(AnalysisResponseModel):
    run_id: UUID
    platform: Platform
    crawler_type: str
    content_count: int = Field(ge=0)
    ...
    analysis_scope: Literal["all_collected_comments"] = "all_collected_comments"
    source_files: list[str] = Field(default_factory=list)
    started_at: datetime
    finished_at: Optional[datetime] = None
    analysis_job_id: Optional[UUID] = None
    analysis_job_status: Optional[str] = None
...
class AnalysisResultScopeListResponse(AnalysisResponseModel):
    items: list[AnalysisResultScopeResponse]
    total: int = Field(ge=0)
