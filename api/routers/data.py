...
ANALYSIS_EXTENSIONS = {".json", ".jsonl", ".csv", ".xlsx"}
PLATFORM_ALIASES = {
    "dy": "douyin",
    "douyin": "douyin",
    "bili": "bilibili",
...
def get_analysis_metadata(file_path: Path) -> dict:
    """Describe whether a historical crawler output can enter the AI workflow."""

    relative = file_path.relative_to(DATA_DIR)
    platform = next(
        (PLATFORM_ALIASES[part.lower()] for part in relative.parts if part.lower() in PLATFORM_ALIASES),
        None,
    )
    extension = file_path.suffix.lower()
    supported = platform is not None and extension in ANALYSIS_EXTENSIONS
    stem = file_path.stem.lower()
    tokens = stem.split("_")
    crawler_type = next((token for token in tokens if token in {"search", "detail", "creator"}), "import")

    # JSON/JSONL/CSV stores content and comments in sibling files. Treat those
    # siblings as one selectable crawl record so analysis retains its context.
    match = re.match(r"^(search|detail|creator)_(comments|contents|creators|videos|notes)_(.+)$", stem)
    if match and extension != ".xlsx":
        group_name = f"{match.group(1)}_{match.group(3)}"
    else:
        group_name = file_path.name.lower()
    group_key = "/".join((platform or "unsupported", relative.parent.as_posix().lower(), group_name))
    group_id = hashlib.sha256(group_key.encode("utf-8")).hexdigest()[:20]
...
def get_file_info(file_path: Path) -> dict:
    ...
    return {
        ...
        **get_analysis_metadata(file_path),
    }
