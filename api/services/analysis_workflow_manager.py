    ...
    async def _execute(self, workflow_id: UUID, request: CollectionAnalysisWorkflowRequest) -> None:
        ...
        try:
            ...
            if items & LLM_ITEMS:
                ...
                successful_count = job.completed_count + job.skipped_count
                self._update(
                    workflow_id,
                    analyzed_count=successful_count,
                    failed_count=job.failed_count,
                    progress_percent=65 if items & SEMANTIC_ITEMS else 90,
                    message=f"评论 AI 分析成功 {successful_count} 条，失败 {job.failed_count} 条",
                )
                if successful_count == 0:
                    raise RuntimeError(
                        f"评论 AI 分析未产出有效结果：{job.failed_count} 条失败。"
                        "请检查 DeepSeek 配置或查看分析任务错误。"
                    )
            ...
            if items & SEMANTIC_ITEMS:
                ...
                if semantic_run.embedded_comments == 0:
                    raise RuntimeError(
                        f"Embedding 未产出有效向量：{semantic_run.failed_comments} 条失败。"
                        "请检查 SiliconFlow 配置。"
                    )
            ...
            self._update(
                ...
                message=(
                    f"分析完成：{completed.analyzed_count} 条评论已有 AI 结果"
                    if items & LLM_ITEMS
                    else "所选本地统计已完成，可以查看下方结果"
                ),
