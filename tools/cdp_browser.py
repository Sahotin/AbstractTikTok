...
class CDPBrowserManager:
    ...
    def _resolve_cdp_user_data_dir(self, *, fresh: bool = False) -> Optional[str]:
        """
        Resolve CDP profile directory.
        Prefer %LOCALAPPDATA% to avoid path/lock issues under non-ASCII project folders.
        """
        ...
        if fresh:
            path = os.path.join(
                tempfile.gettempdir(),
                "MediaCrawler",
                f"{profile_name}_{int(time.time())}",
            )
        else:
            local_app = os.environ.get("LOCALAPPDATA")
            if local_app:
                base = os.path.join(local_app, "MediaCrawler", "browser_data")
            else:
                base = os.path.join(os.getcwd(), "browser_data")
            path = os.path.join(base, profile_name)

        os.makedirs(path, exist_ok=True)
        self._clear_stale_profile_lock(path)
        return path

    @staticmethod
    def _clear_stale_profile_lock(user_data_dir: str) -> None:
        lock_file = os.path.join(user_data_dir, "SingletonLock")
        if os.path.isfile(lock_file):
            try:
                os.remove(lock_file)
                utils.logger.info(
                    f"[CDPBrowserManager] Removed stale profile lock: {lock_file}"
                )
            except OSError as e:
                utils.logger.warning(
                    f"[CDPBrowserManager] Could not remove profile lock {lock_file}: {e}"
                )
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        """
        user_data_dir = self._resolve_cdp_user_data_dir()
        if user_data_dir:
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0:
                    utils.logger.warning(
                        f"[CDPBrowserManager] Browser launch failed ({e}), "
                        "retrying with a fresh temporary profile..."
                    )
                    user_data_dir = self._resolve_cdp_user_data_dir(fresh=True)
                    if user_data_dir:
                        utils.logger.info(
                            f"[CDPBrowserManager] Retry user data directory: {user_data_dir}"
                        )
                    await self.cleanup()
                    self.debug_port = self.launcher.find_available_port(config.CDP_DEBUG_PORT)
                else:
        ...
        raise RuntimeError(
            ...
            "Please close all Chrome windows and try again, or set ENABLE_CDP_MODE=False "
            "in config/base_config.py to use built-in Playwright browser."
        ) from last_error
    ...
    async def _launch_browser_once(
        ...
        """Single attempt to launch browser and wait for CDP port."""
