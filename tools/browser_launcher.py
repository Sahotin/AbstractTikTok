...
class BrowserLauncher:
    ...
    def launch_browser(self, browser_path: str, debug_port: int, headless: bool = False,
        ...
        args = [
            ...
            "--remote-allow-origins=*",  # Required for Chrome 111+ CDP WebSocket
            "--no-first-run",
    ...
    def _is_debug_port_open(self, debug_port: int) -> bool:
        """Check whether Chrome CDP port is accepting connections."""
        for host in ("127.0.0.1", "localhost"):
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(1)
                    if s.connect_ex((host, debug_port)) == 0:
                        return True
            except Exception:
                pass
        return False
    ...
    def wait_for_browser_ready(self, debug_port: int, timeout: int = 30) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            if proc and proc.poll() is not None and not warned_exit:
                warned_exit = True
                utils.logger.warning(
                    f"[BrowserLauncher] Browser launcher process exited (code={proc.returncode}); "
                    "waiting for CDP port (Chrome may have spawned a child process)..."
                )
        ...
        if proc and proc.poll() is not None:
            utils.logger.error(
                f"[BrowserLauncher] Browser process exited (code={proc.returncode}) "
                f"and CDP port {debug_port} never opened"
            )
