...
from .help import extract_douyin_url_from_text, parse_video_info_from_url, parse_creator_info_from_url
...
class DouYinCrawler(AbstractCrawler):
    ...
    async def get_specified_awemes(self):
        ...
        for video_url in config.DY_SPECIFIED_ID_LIST:
            try:
                normalized_url = extract_douyin_url_from_text(video_url)
                video_info = parse_video_info_from_url(normalized_url)

                # Handling short links
                if video_info.url_type == "short":
                    utils.logger.info(f"[DouYinCrawler.get_specified_awemes] Resolving short link: {normalized_url}")
                    resolved_url = await self.dy_client.resolve_short_url(normalized_url)
                    if resolved_url:
                        ...
                        utils.logger.error(f"[DouYinCrawler.get_specified_awemes] Failed to resolve short link: {normalized_url}")
                        continue

                aweme_id_list.append(video_info.aweme_id)
                utils.logger.info(f"[DouYinCrawler.get_specified_awemes] Parsed aweme ID: {video_info.aweme_id} from {normalized_url}")
