...
# Match Douyin links embedded in App share text (e.g. "复制打开抖音... https://v.douyin.com/xxx/")
...
def extract_douyin_url_from_text(text: str) -> str:
    """
    Extract the first Douyin URL from share text, or return stripped input unchanged.
    """
    text = text.strip()
...
def parse_video_info_from_url(url: str) -> VideoUrlInfo:
    ...
    5. App share text containing a link (auto-extracts https://v.douyin.com/...)

    Args:
        url: Douyin video link, share text, or ID
    Returns:
        VideoUrlInfo: Object containing video ID
    """
    url = extract_douyin_url_from_text(url)
    ...
    is_short = "v.douyin.com" in url or (
        url.startswith("http") and len(url) < 50 and "video" not in url
    )
    if is_short:
...
if __name__ == '__main__':
    ...
    test_urls = [
        ...
        "7525082444551310602",
        "9.23 复制打开抖音 https://v.douyin.com/3t6YnTbiLQI/ K@W.Mw 11/11 :5pm RXZ:/",
    ]
